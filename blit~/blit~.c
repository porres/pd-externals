...
static t_int *blit_tilde_perform(t_int *w) {
    ...
    int h = x->x_h;             // recall these values from the previous block
    int max_h = x->x_max_h;     //
    double incr;

    while (n--) {
        incr = *in1 * M_PI * speriod; // phase increment
        if (phase == 0 || phase == M_PI) // avoid divide by 0
            *out++ = 1.0;
        else if (h == 0)
            *out++ = 0.0;
        else if (*in3 >= 0 && (h % 2) == 0 && h > 1) // odd harmonics
            *out++ = (sin(phase * (h - 1)) / sin(phase)) / (h - 1);
        else if (*in3 < 0 && (h % 2) == 1 && h > 1) // even harmonics
            *out++ = (sin(phase * (h - 1)) / sin(phase)) / (h - 1);
        else
            *out++ = (sin(phase * h) / sin(phase)) / h;
        phase += incr;
        if (phase >= TWOPI) { // ph wrap-around and ph-locked harmonic variation
            h = *in2 > 0 ? (2 * (*in2)) : (-2 * (*in2)); // number of harmonics
